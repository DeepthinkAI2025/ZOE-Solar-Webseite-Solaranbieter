    ...
    return {
      ...
      terserOptions: {
        compress: {
          ...
          pure_funcs: mode === 'production' ? ['console.log', 'console.info', 'console.debug', 'console.warn', 'console.error', 'console.trace', 'console.time', 'console.timeEnd', 'console.group', 'console.groupEnd'] : [],
          dead_code: true,
          sequences: true,
          conditionals: true,
          booleans: true,
        ...
        format: {
          comments: false,
          quote_style: 1, // Single quotes for smaller bundle
          wrap_iife: true
        }
      },
      rollupOptions: isSSR ? {
        ...
        output: {
          manualChunks: (id) => {
            if (id.includes('node_modules')) {
              // CRITICAL PATH: React & Router bundled together
              if (id.includes('react') || id.includes('react-dom') || id.includes('react-router')) {
                return 'react-vendor';
              }
              // Ultra-aggressive bundling for performance
      ...
      cssCodeSplit: true,
      assetsInlineLimit: 4096, // REDUCED for better performance
      chunkSizeWarningLimit: 1500, // LOWER for better performance
      modulePreload: {
        polyfill: false,
