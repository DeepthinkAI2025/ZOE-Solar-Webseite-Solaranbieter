...
async function embed(texts) {
  if (EMBEDDING_PROVIDER === 'gemini') {
    const model = process.env.GEMINI_MODEL || 'textembedding-gecko-001';
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        console.error(`Gemini/Google Embedding Auth Fehler (${status}). Prüfe Service-Account / API‑Key & Berechtigungen. Antwort:`, respData);
      ...
      console.error('Gemini-Embedding-Fehler:', respData || err.message || err);
      throw err;
    }
  } else {
