...
import { multiChannelAttributionService as multiChannelAttributionModelingService } from './multiChannelAttributionService';
...
class AEOIntegrationService {
  ...
  private async syncEntityKnowledgeGraph(): Promise<SyncResult> {
    ...
    for (const entity of entities) {
      if (typeof entity === 'object' && entity !== null && (entity as any).type === 'Place' && this.config.integration.localSEO) {
        await this.updateLocalSEOWithEntity(entity as any);
        updatedCount++;
      }
    }
  ...
  const automation = marketingAutomationIntegrationService.getAutomationMetrics();
    let updatedCount = 0;
  ...
  private getRelevantEntities(url: string): string[] {
    ...
    if (url.includes('/photovoltaik')) {
      return entities
        .filter((e: any) => e && typeof e === 'object' && e.name && e.name.toLowerCase().includes('photovoltaik') || e.categories?.includes('Photovoltaik'))
        .map((e: any) => e && e.id ? e.id : '');
    }

    return entities.slice(0, 5).map((e: any) => e && e.id ? e.id : '');
  }
