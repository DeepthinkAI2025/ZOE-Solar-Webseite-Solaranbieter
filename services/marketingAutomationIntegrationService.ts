...
Kostenlose Beratung vor Ort!`,
  ...
  public getAutomationMetrics(): {
    timestamp: string;
    globalDashboard: ReturnType<MarketingAutomationIntegrationService['getGlobalMarketingDashboard']>;
    totals: {
      campaigns: number;
    ...
    locationInsights: Array<{
      ...
      topCampaign?: {
        ...
        name: string;
        roi: number;
        status: LocalMarketingCampaign['status'];
      };
      recommendations: string[];
    }>;
    recommendations: string[];
  } {
    const globalDashboard = this.getGlobalMarketingDashboard();

    const locationInsights = PRIMARY_SERVICE_REGIONS.map(region => {
      const locationKey = region.city.toLowerCase();
      const campaigns = this.getMarketingCampaigns(locationKey);
      const automations = this.marketingAutomations.get(locationKey) || [];
      const intelligence = this.marketingIntelligence.get(locationKey);

      const activeCampaigns = campaigns.filter(c => c.status === 'active').length;
      const activeAutomations = automations.filter(a => a.status === 'active').length;
      const avgROI = campaigns.length > 0
        ? campaigns.reduce((sum, campaign) => sum + campaign.performance.metrics.roi, 0) / campaigns.length
        : 0;
      const topCampaign = campaigns.length > 0
        ? [...campaigns].sort((a, b) => b.performance.metrics.roi - a.performance.metrics.roi)[0]
        : undefined;
      ...
      return {
        ...
        topCampaign: topCampaign
          ? {
              ...
              roi: topCampaign.performance.metrics.roi,
              status: topCampaign.status
            }
          : undefined,
        recommendations: intelligence?.recommendations?.map(rec => rec.recommendation) || []
      };
    });

    const totals = locationInsights.reduce((acc, insight) => {
      acc.campaigns += insight.campaigns;
      acc.activeCampaigns += insight.activeCampaigns;
      acc.automations += insight.automations;
      acc.activeAutomations += insight.activeAutomations;
      return acc;
    }, {
      campaigns: 0,
      activeCampaigns: 0,
    ...
    const recommendations = [
      ...globalDashboard.recommendations,
      ...locationInsights.flatMap(insight => insight.recommendations)
