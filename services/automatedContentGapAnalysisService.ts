...
class AutomatedContentGapAnalysisService {
  ...
  private _startGapAnalysis(): void {
  ...
  private async generateKeywordContentSuggestions(keywordData: KeywordGap): Promise<Array<{ title: string; type: string; wordCount: number; keywords: string[]; estimatedPerformance: number }>> {
    ...
    const aiTitleResp = await optimizeKeywords({ content: titlePrompt });

    suggestions.push({
      title: (aiTitleResp && typeof aiTitleResp === 'object' && 'optimizedTitle' in aiTitleResp && aiTitleResp.optimizedTitle)
        ? aiTitleResp.optimizedTitle
        : `Alles über ${keywordData.keyword}`,
      type: (['blog','case-study','guide','tool','video'].includes(contentType) ? contentType : 'blog') as 'blog'|'case-study'|'guide'|'tool'|'video',
      wordCount,
  ...
  private async generateAISuggestions(gap: ContentGap): Promise<Array<{ title: string; type: 'blog' | 'guide' | 'case-study' | 'tool' | 'video'; wordCount: number; keywords: string[]; estimatedPerformance: number }>> {
    const suggestions = [];
