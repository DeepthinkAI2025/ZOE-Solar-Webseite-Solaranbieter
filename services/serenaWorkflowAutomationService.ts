 ...
 * Nutzt kostenlose Automatisierungs-Tools für Entwicklung und Deployment
 */
...
class SerenaWorkflowAutomationService {
  ...
  private getTestSuiteByCommand(command: string): TestSuite | undefined {
    for (const testSuite of Array.from(this.testSuites.values())) {
  ...
  public addTestSuite(testSuite: TestSuite): void {
    ...
    console.log(`✅ Added test suite: ${testSuite.name}`);
  }
