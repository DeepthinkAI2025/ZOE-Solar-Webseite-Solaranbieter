...
const Icon: React.FC<{ name: string }> = ({ name }) => {
    const className = "h-12 w-12 text-green-500 mb-6";
...
const InfoCard: React.FC<{ iconName: string; title: string; text: string; linkText: string; onClick: () => void; isFeatured?: boolean }> = ({ iconName, title, text, linkText, onClick, isFeatured }) => (
    <div className={`group relative p-8 rounded-2xl border-2 flex flex-col transition-all duration-300 ${isFeatured ? 'bg-slate-800 border-slate-700 text-white shadow-2xl lg:scale-105' : 'bg-white border-slate-200 hover:border-green-300 hover:shadow-xl hover:-translate-y-2'}`}>
        <Icon name={iconName} />
        ...
        <button onClick={onClick} className={`mt-auto font-bold group-hover:text-green-700 transition-colors ${isFeatured ? 'text-green-400 group-hover:text-white' : 'text-green-600'}`}>
