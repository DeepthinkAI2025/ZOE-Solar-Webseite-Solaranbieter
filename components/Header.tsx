...
const Header: React.FC<HeaderProps> = ({
  ...
  const dropdownActiveClass = isNight
    ? 'text-emerald-200 !border-b-transparent'
    : 'text-primary-600 !border-b-transparent';
  ...
  const navLinkClasses = (page: Page | 'leistungen_parent' | 'produkte_parent' | 'wissen_parent' | 'preise_parent') => {
    ...
    const activeClasses = isNight
      ...
      : 'font-semibold text-primary-600 border-primary-600';
    const inactiveClasses = isNight
      ? 'border-transparent text-slate-300 hover:text-emerald-200 hover:border-emerald-300/60'
      : 'border-transparent text-slate-600 hover:text-primary-600 hover:border-primary-300';
        ...
        <NavLink
          ...
          className={`hidden lg:flex items-center gap-2 rounded-full px-3 py-1.5 text-sm transition-all duration-300 cursor-pointer group ${
            isNight
              ...
              : 'bg-slate-100 border border-slate-200 text-slate-600 hover:bg-primary-50 hover:border-primary-300 hover:text-primary-700'
          }`}
        ...
        >
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <NavLink
              ...
              className={`${navLinkClasses('sonderaktionen')} text-accent-500 font-bold border-accent-500 hover:text-accent-600 hover:border-accent-400`}
            >
      ...
      <div className={`flex items-center gap-3 pl-4 border-l ${isNight ? 'border-slate-700' : 'border-slate-200'}`}>
        ...
        <button onClick={openCommandHub} title="Suchen (Cmd+K)" aria-label="Suchen (Cmd+K)" className={`p-2 rounded-full transition-colors duration-200 ${isNight ? 'text-slate-200 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-200'}`}>
                ...
                <div ref={ctaDropdownRef} className="relative">
                    <button onClick={() => setIsCtaDropdownOpen(prev => !prev)} className="bg-primary-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-primary-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 cursor-pointer cta-button-glow flex items-center gap-2">
                      Jetzt anfragen
           ...
           <div className={`absolute top-full right-0 mt-2 w-72 backdrop-blur-lg rounded-md overflow-hidden transition-all duration-300 transform origin-top-right ${
            ...
            <button
              ...
              className={`w-full text-left group flex items-start gap-4 p-4 transition-colors duration-200 cursor-pointer ${
                isNight ? 'hover:bg-emerald-500/10' : 'hover:bg-primary-50'
              }`}
            >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-primary-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                            <div>
                <h4 className={`font-bold ${isNight ? 'text-slate-100 group-hover:text-emerald-200' : 'text-slate-800 group-hover:text-primary-600'}`}>Mit KI-Berater sprechen</h4>
            ...
            <a
              ...
              className={`w-full text-left group flex items-start gap-4 p-4 transition-colors duration-200 cursor-pointer border-t ${
                isNight
                  ...
                  : 'border-slate-200/80 hover:bg-primary-50'
              }`}
            >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-primary-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                            <div>
                <h4 className={`font-bold ${isNight ? 'text-slate-100 group-hover:text-emerald-200' : 'text-slate-800 group-hover:text-primary-600'}`}>Kundenservice anrufen</h4>
      ...
      >
        <div className={`p-8 h-full flex flex-col mobile-menu-is-open`} style={{ paddingTop: `${bannerHeight + 88}px` }}>
            <div className="space-y-2 mb-8">
                ...
                <Link to="/preise" onClick={() => handleLinkClick('preise')} className="nav-item-stagger block text-slate-700 hover:text-primary-600 cursor-pointer text-2xl font-semibold px-4 py-3 rounded-md hover:bg-slate-100">Preise</Link>
                <Link to="/leistungen" onClick={() => handleLinkClick('leistungen')} className="nav-item-stagger block text-slate-700 hover:text-primary-600 cursor-pointer text-2xl font-semibold px-4 py-3 rounded-md hover:bg-slate-100">Leistungen</Link>
                <Link to="/produkte" onClick={() => handleLinkClick('produkte')} className="nav-item-stagger block text-slate-700 hover:text-primary-600 cursor-pointer text-2xl font-semibold px-4 py-3 rounded-md hover:bg-slate-100">Produkte</Link>
                <Link to="/wissens-hub" onClick={() => handleLinkClick('wissens-hub')} className="nav-item-stagger block text-slate-700 hover:text-primary-600 cursor-pointer text-2xl font-semibold px-4 py-3 rounded-md hover:bg-slate-100">Wissen</Link>
                <Link to="/kontakt" onClick={() => handleLinkClick('kontakt')} className="nav-item-stagger block text-slate-700 hover:text-primary-600 cursor-pointer text-2xl font-semibold px-4 py-3 rounded-md hover:bg-slate-100">Kontakt</Link>
            </div>
               ...
               {isLoggedIn ? (
                    ...
                    <>
                      ...
                      <div ref={mobileCtaRef}>
                          <button onClick={() => setIsMobileCtaOpen(!isMobileCtaOpen)} className="w-full bg-primary-600 text-white font-bold py-4 px-6 rounded-xl hover:bg-primary-700 transition-all duration-300 shadow-lg flex justify-center items-center gap-2">
                              Jetzt anfragen
