...
const AIRecommender: React.FC = () => {
    ...
    const getNextQuestion = async (userAnswer?: string) => {
        ...
        const systemPrompt = `Du bist "ZOE", ein freundlicher und kompetenter KI-Produktberater für ZOE Solar. Deine Aufgabe ist es, Kunden durch 2-3 gezielte Fragen zu führen, um ihnen das am besten passende Solarpaket aus unserer Liste zu empfehlen.
...
Antworte NUR mit gültigem JSON.`;
        ...
        const prompt = `Analysiere das bisherige Gespräch und antworte entsprechend.`;
        ...
        try {
            const response: AIResponse = await openRouterClient.generateContent({
                ...
                temperature: 0.3,
                maxTokens: 500,
                context: `Verfügbare Pakete: ${pricingPackages.map(p => p.id).join(', ')}`
            });
