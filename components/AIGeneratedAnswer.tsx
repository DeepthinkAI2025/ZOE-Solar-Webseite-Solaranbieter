 ...
 * Speziell optimiert für Google Gemini, AI Overview und SGE (Search Generative Experience)
...
interface AIGeneratedAnswerProps {
  data: AIAnswerData;
  variant?: 'featured-snippet' | 'ai-overview' | 'sge-optimized' | 'gemini-ready';
  showSources?: boolean;
...
const AIGeneratedAnswer: React.FC<AIGeneratedAnswerProps> = ({
  ...
  const generateStructuredData = () => {
    ...
    if (variant === 'ai-overview' || variant === 'gemini-ready') {
      structuredData.mainEntity[0].acceptedAnswer.aiGenerated = true;
      structuredData.mainEntity[0].acceptedAnswer.contextualInformation = data.context;
      structuredData.mainEntity[0].acceptedAnswer.expertiseLevel = data.expertiseLevel;

      if (locationSpecific && userLocation) {
        structuredData.mainEntity[0].acceptedAnswer.locationSpecificity = {
          "@type": "Place",
          "name": userLocation
        };
      }
