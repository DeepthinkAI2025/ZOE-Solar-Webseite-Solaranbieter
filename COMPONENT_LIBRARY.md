...
```tsx
// Primary (uses brand primary/energy token)
...
**Variants:**
- Primary (brand primary-600 background)
- Secondary (brand primary-600 border)
- Tertiary (text only)
...
## Design Tokens & Farb-Utilities

- Brand-Primärfarbe: wird als `primary`/`energy`-Palette in Tailwind konfiguriert.
- Verwende in Komponenten konsequent Klassen wie `bg-primary-600`, `text-primary-600`, `border-primary-600` anstelle von `bg-green-600` etc.
- Focus/Outline: global über CSS-Variable `--primary` definiert, Tailwind-kompatibel über `focus:ring-primary-*`.

**Beispiel:**

<augment_code_snippet mode="EXCERPT">
````tsx
<button className="inline-flex items-center justify-center rounded-lg bg-primary-600 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500">
  Primäre Aktion
</button>
````
