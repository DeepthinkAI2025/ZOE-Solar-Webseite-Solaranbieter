...
// Renamed: previously callGeminiWithRetry — now provider-agnostic and used for OpenRouter
...
const ConfigSlider: React.FC<ConfigSliderProps> = ({ field, onConfirm }) => {
    ...
    if (!field) {
        return null; // Guard clause for undefined field
    }
    return (
        <div className="space-y-3 text-sm animate-fade-in p-3 bg-slate-100 rounded-2xl">
...
const AIChatFunnel: React.FC<AIChatFunnelProps> = ({ onOpen, currentPage, initialContext }) => {
    ...
    // Use OpenRouterClient singleton
    ...
    const speak = useCallback((text: string) => {
        ...
        const processQueue = () => {
            ...
            utterance.voice = currentLang === 'de-DE' ? (germanVoice ?? null) : (englishVoice ?? null);
    ...
    useEffect(() => {
    // ai.current = new GoogleGenAI({ apiKey: process.env.API_KEY as string }); // Removed
        ...
        if (startType === 'promo') {
            ...
            if (service.configurableFields && service.configurableFields.length > 0) {
             ...
             if (context.type === 'contextual_help') {
                ...
                const innovation = innovations.find((i: any) => i.id === context);
                if (innovation) {
...
- **Mögliche Module:** ca. ${roofAnalysisData.estimatedModuleCount} Stk.
        `;
    setFormData((prev: any) => ({...prev, message: (prev.message || '') + '\n\n' + analysisText}));
        ...
        try {
            ...
            for (const zoom of zoomLevels) {
                ...
                const validationSchema = {
                    type: 'object',
                    properties: {
                        isValid: { type: 'boolean', description: "True, if the image is a clear, high-quality satellite view of a single building's roof." },
                        reason: { type: 'string', description: "Reason for the decision. E.g., 'Too blurry', 'Multiple buildings shown', 'Clear view of a single roof'." }
                    },
                ...
                const validationApiCall = () => openRouterClient.generateContent({
                    prompt: `[IMAGE DATA: ${base64data || ''}]\n\n${validationPrompt}`,
                });
    
                try {
                    const validationResponse = await callOpenRouterWithRetry<any>(validationApiCall, 2); // Fewer retries for validation
                    const validationResult = JSON.parse(validationResponse.text!);
                    // Validation result received for zoom level
    
                    if (validationResult.isValid && base64data) {
            ...
            const analysisSchema = {
              type: 'object',
              properties: {
                analysisPossible: { type: 'boolean', description: "Gibt an, ob eine sinnvolle Analyse des Bildes möglich war." },
                usableAreaSqm: { type: 'number', description: "Geschätzte nutzbare Dachfläche in Quadratmetern. 0, wenn Analyse nicht möglich." },
                obstructions: { type: 'array', description: "Eine Liste der erkannten Störelemente. Leer, wenn Analyse nicht möglich.", items: { type: 'string' } },
                estimatedModuleCount: { type: 'number', description: "Geschätzte Anzahl an Standard-Solarmodulen. 0, wenn Analyse nicht möglich." },
              },
            ...
            const analysisApiCall = () => openRouterClient.generateContent({
                prompt: `[IMAGE DATA: ${bestImage?.base64 || ''}]\n\n${analysisPrompt}`,
            });
    
                const analysisResponse = await callOpenRouterWithRetry<any>(analysisApiCall);
    ...
    if (!openRouterClient) return;
        ...
        const prompt = `Sie sind ein erstklassiger Experte für Photovoltaik-Technologie und Berater für ZOE Solar. Ein Kunde möchte die folgenden Produkte aus der Kategorie "${products[0]?.category || 'Solarprodukte'}" vergleichen.
...
3.  **Empfehlung & Fazit:** Geben Sie eine klare, begründete Empfehlung. Erklären Sie, welches Produkt für welchen Kundentyp (z.B. "preisbewusst", "leistungsorientiert", "auf Langlebigkeit bedacht") die beste Wahl ist. Schließen Sie mit einem zusammenfassenden Satz.`;
        ...
        try {
            const apiCall = () => openRouterClient.generateContent({
                prompt: prompt,
            });
            const response: any = await callOpenRouterWithRetry(apiCall);
    ...
    - Typische Herausforderungen: ${useCase.challenges.map((c: any) => c.title).join(', ')}
    - Unsere Lösungen: ${useCase.solutions.map((s: any) => s.title).join(', ')}
            ...
            properties: {
                questions: {
                    ...
                    items: {
                        type: 'object',
                        properties: {
                            questionText: { type: 'string', description: "Die Frage an den Kunden." },
                            options: { type: 'array', description: "Eine Liste von 3-4 Multiple-Choice-Antworten.", items: { type: 'string' } }
                        },
    ...
    setFormData((prev: any) => ({ ...prev, [currentField.id]: value, message: `${prev.message || ''} ${currentField.label}: ${value} ${currentField.unit}\n` }));
    ...
    const handleUserInput = async (input: string) => {
        ...
        switch(step) {
            case 'start':
                ...
                break;
            case 'inquiry_type':
                setFormData((prev: any) => ({ ...prev, userType: input.toLowerCase().includes('gewerblich') ? 'commercial' : 'private' }));
                goToStep('service_type');
                break;
            case 'service_type':
                setFormData((prev: any) => ({ ...prev, serviceType: input }));
                goToStep('get_name');
                break;
            case 'get_address':
                shouldSetLoadingFalse = false;
            ...
            case 'asking_ai_questions':
                ...
                setFormData((prev: any) => ({ ...prev, [answerKey]: input }));
                ...
                break;
            case 'get_name':
                 setFormData((prev: any) => ({ ...prev, contactPerson: input }));
                 goToStep('get_email');
                 break;
            case 'get_email':
                if (!input.includes('@') || !input.includes('.')) {
                    ...
                    setFormData((prev: any) => ({ ...prev, email: input }));
                    goToStep('get_phone');
                }
                break;
            case 'get_phone':
            ...
            case 'get_phone_for_callback':
                setFormData((prev: any) => ({ ...prev, phone: input }));
    ...
    const serviceKnowledge = services.map((s: any) => ({ id: s.id, title: s.title, context: s.context }));
        ...
        **Wichtige Anweisungen:**
        ...
        try {
            // Workaround für Streaming: Verwende normale generateContent und simuliere Streaming
            const streamResult = await openRouterClient.generateContent({
                prompt: prompt,
            }) as any;
            ...
            const finalResponse = streamResult as any;
            ...
            let cleanText = videoId ? fullResponseText.replace(youtubeRegex, '').trim() : fullResponseText;
             ...
             try {
                ...
                if (jsonResponse.serviceId) {
                    const matchedService = services.find((s: any) => s.id === jsonResponse.serviceId);
// Add missing type stubs for compatibility
type GenerateContentResponse = any;
// If you need a more specific type, define it here.
// Type is replaced with string literals in this patch.
                    if (matchedService) {
                        setIsLoading(false);
    ...
    return (
        <>
            <button 
                ...
                className={`fixed bottom-6 right-6 z-[110] flex items-center gap-3 h-16 px-6 rounded-full bg-green-600 text-white font-bold shadow-2xl transition-all duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-lg hover:shadow-green-500/40 focus:outline-none focus:ring-4 focus:ring-green-300 
            ...
            >
                ...
                <span className="hidden sm:inline">Wie kann ich helfen?</span>
            </button>
