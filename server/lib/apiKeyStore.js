...
function mergeWithDefaults(currentState = {}) {
  return {
    ...
    ...currentState,
    openrouter: {
      ...defaultState.openrouter,
      ...(currentState.openrouter ?? {})
    },
    googleMaps: {
...
export function summariseKeys(state) {
  ...
  return {
  openrouter: {
      configured: Boolean(summary.openrouter.apiKey),
      mask: maskValue(summary.openrouter.apiKey),
      model: summary.openrouter.model,
      lastUpdated: summary.openrouter.lastUpdated
    },
    googleMaps: {
      configured: Boolean(
...
export function updateService(state, service, payload) {
  ...
  switch (service) {
    case 'openrouter': {
      updated.openrouter.apiKey = payload.apiKey ?? '';
      updated.openrouter.model = payload.model ?? updated.openrouter.model;
      updated.openrouter.lastUpdated = timestamp;
      break;
    }
    case 'googleMaps': {
