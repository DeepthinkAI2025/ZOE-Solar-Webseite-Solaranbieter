import { evaluateAssetCandidates, isGeminiConfigured } from './providers/geminiProvider.js';
...
export async function analyseAssetCandidates(manufacturer, { logoCandidates = [], datasheetCandidates = [] } = {}) {
  ...
  if (isGeminiConfigured() && (accessibleLogos.length || accessibleDatasheets.length)) {
    try {
      ...
      geminiUsed = approvedLogos.length > 0 || approvedDatasheets.length > 0;
    } catch (err) {
