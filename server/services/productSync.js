...
import { openRouterProvider, isOpenRouterConfigured } from './providers/openrouterProvider.js';
...
export async function runProductSync({ manufacturers = [] } = {}) {
  ...
  if (base.website && isOpenRouterConfigured() && fallbackNeeded) {
  const openrouterStarted = Date.now();
      try {
  const ai = await openRouterProvider({ slug: base.slug, name: base.name, website: base.website });
        base.syncMeta = {
          ...base.syncMeta,
          openrouterUsed: true,
          openrouterDurationMs: Date.now() - openrouterStarted
        };
  base.providersUsed = dedupeProviders(base.providersUsed, ['openrouter']);
        ...
        if (Array.isArray(ai?.logoCandidates) && ai.logoCandidates.length > 0) {
          ...
          for (const candidate of ai.logoCandidates) {
            ...
            try {
              ...
              console.warn('[productSync] Failed to cache OpenRouter logo candidate for', base.slug, logoErr.message || logoErr);
            }
          }
        }
  } catch (err) {
        ...
        console.warn('[productSync] OpenRouter fallback failed for', base.slug, err?.message ?? err);
      }
    }
  ...
  const openrouterUsedCount = enrichedManufacturers.filter((m) =>
    Array.isArray(m?.providersUsed) && m.providersUsed.includes('openrouter')
  ).length;
