...
// Dynamische Test-URL basierend auf Environment
const getTestUrl = (): string => {
  const envUrl = process.env.TEST_URL || process.env.NEXT_PUBLIC_TEST_URL;
  if (envUrl) {
    return envUrl;
  }

  // Fallback basierend auf Environment
  if (process.env.NODE_ENV === 'production') {
    return 'https://zoe-solar.de';
  }
